...
function UI_Start_C:Initialize(Initializer)
    self.ProfData = {}
    self.ProfData[0] = {Mesh='/Game/Role/Swat/Swat.Swat', Anim='/Game/Blueprint/Swat/ABP_Swat.ABP_Swat_C', RTT='/Game/UI/RT/RTT_Character.RTT_Character', Prof=1}
    self.ProfData[1] = {Mesh='/Game/Role/Sophie/Sophie.Sophie', Anim='/Game/Blueprint/Sophie/ABP_Sophie.ABP_Sophie_C', RTT='/Game/UI/RT/RTT_Character_2.RTT_Character_2', Prof=2}
    self.ProfData[2] = {Mesh='/Game/Role/Alex/Alex.Alex', Anim='/Game/Blueprint/Alex/ABP_Alex.ABP_Alex_C', RTT='/Game/UI/RT/RTT_Character_3.RTT_Character_3', Prof=3}

    self.PreviewActorClass = "/Game/BP_New/Record3D_Swat.Record3D_Swat_C"
    self.PreviewWeaponClass = "/Game/BP_New/Weapon/BP_Weapon.BP_Weapon_C"
end
...
function UI_Start_C:CreatePlayer(TheWorld, PlayerClass, Translation, ProfData)
    ...
    local SKMC = Player.SkeletalMesh
    local Mesh = UE4.UObject.Load(ProfData.Mesh)


    --SKMC.SkeletalMesh = Mesh
    Player.SkeletalMesh:SetSkeletalMeshAsset(Mesh,false)
    local AnimAsset = UE.UClass.Load(ProfData.Anim)

    SKMC.AnimAsset = AnimAsset
    --Player.SkeletalMesh:SetAnimClass(AnimAsset)
    

    local texture = UE.UObject.Load(ProfData.RTT)
    Player.SceneCapture.TextureTarget = texture

	local WeaponClass = UE.UClass.Load(self.PreviewWeaponClass)
