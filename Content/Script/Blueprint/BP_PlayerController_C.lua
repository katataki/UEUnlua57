...
function BP_PlayerController_C:Fire_Pressed()
    ...
    if MyInterface then 
        --MyInterface.StartFire(self.Pawn)
        self.Pawn:StartFire()
    end
end
...
function BP_PlayerController_C:Fire_Released()
    ...
    local MyInterface = UE.UClass.Load("/Game/BPI_Interfaces.BPI_Interfaces_C")
    if MyInterface then 
        MyInterface.StopFire(self.Pawn)
    end
end
...
function BP_PlayerController_C:ShowChat()
    local UCLass = UE4.UClass.Load('/Game/UI/UIChat.UIChat_C')
    if UCLass == nil then
        return
    end
...
function BP_PlayerController_C:Event_ShowLobbyUI_RPC()
    ...
    local UCLass = UE4.UClass.Load('/Game/UI/UI_Lobby.UI_Lobby_C')
    if UCLass == nil then
        return
    end
    local child = UE4.UWidgetBlueprintLibrary.Create(self, UCLass, nullptr)
...
function BP_PlayerController_C:Event_showLoading_RPC()
    local UCLass = UE4.UClass.Load('/Game/UI/UI_Loading.UI_Loading_C')
    if UCLass == nil then
        return
    end
    local child = UE4.UWidgetBlueprintLibrary.Create(self, UCLass, nullptr)
